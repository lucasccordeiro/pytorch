...
\begin{frame}{Problem \& threat model}
  ...
  \begin{itemize}
    ...
    \item We want \textbf{exhaustive} equivalence over an input region, with
      \textbf{bit-precise IEEE-754} (where fusion breaks: reassociation, rounding, NaN/Inf).
...
\begin{frame}{Running example: QKV projection}
  ...
  unfused projection $\Rightarrow$ the two are equal \textbf{bit-for-bit} in IEEE-754, not merely
  within a tolerance. Both claims are checked.
\end{frame}
...
\begin{frame}{The two programs, side by side}
  ...
  \begin{tikzpicture}[
    ...
    \node[draw=ok,rounded corners,fill=ok!8,text=ok,font=\footnotesize\bfseries,
          right=8mm of Q,yshift=-13mm,align=center] {$\equiv$ \\ bit-for-bit \\ equal};
  \end{tikzpicture}
\end{frame}

% ---------------------------------------------------------------
\begin{frame}[fragile]{The embedding: how the program enters the verifier}
  The harness \textbf{is} the program --- no separate spec to maintain.
  \begin{itemize}
    \item \textbf{Tensors} $\to$ nested lists; \textbf{ops} $\to$ operational model \texttt{torch.py}
      (\texttt{mm/matmul/cat/split/allclose}), pure-Python, float-typed, FLAIL-baked.
...
\begin{frame}{Results}
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    \texttt{*\_buggy} ($\times 3$) & refutation & \bad\ FAILED (+\,c.ex.) & 35--338 s \\
...
\begin{frame}{ESBMC (BMC) vs Lean (ITP) {+} LLM agent}
  ...
  \begin{tabular}{@{}p{0.2\textwidth} p{0.37\textwidth} p{0.37\textwidth}@{}}
    ...
    \midrule
    Embedding effort & program \textbf{is} the spec (write both, assert $\equiv$) & embed tensor {+} FP semantics, \textbf{state} the theorem \\
...
\begin{frame}{Status $\cdot$ what's next}
  \begin{itemize}
    ...
    \item $\rightarrow$ A like-for-like \textbf{Lean {+} agent} encoding of the same equivalence, to
      quantify the encoding/proof effort empirically.
  \end{itemize}
\end{frame}
...
\begin{frame}[plain]
  ...
  interactive proof buys generality at a higher encoding/proof cost and \textbf{no counterexamples}.
  \vfill
\end{frame}}
