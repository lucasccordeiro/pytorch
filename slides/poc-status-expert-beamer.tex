...
\begin{frame}{Problem \& threat model}
  ...
  \end{itemize}
  \medskip
  {\footnotesize Checked under a \textbf{sequential IEEE-754 semantics} (the operational model).
  \textbf{Not modelled:} backend-specific optimisations (FMA, cuBLAS), parallel/tree reductions,
...
\begin{frame}{Pipeline \& soundness}
  ...
  \begin{itemize}
    ...
    \item \textbf{Not modelled}: backend-specific optimisations (FMA, parallel/tree reductions,
      cuBLAS) --- we verify the reference operational semantics, not a specific GPU kernel.
  \end{itemize}
\end{frame}
