...
\begin{frame}{Pipeline \& soundness}
  ...
  \begin{itemize}
    ...
    \item TCB: Python frontend $+$ operational model $+$ SMT solver. \textbf{OM validated} vs real
      PyTorch (\texttt{validation/}): \texttt{cat}/\texttt{split}/\texttt{allclose} bit-exact,
      \texttt{mm} within $\sim$4e-13 (torch's reduction order differs).
...
% ---------------------------------------------------------------
\begin{frame}{Results}
  \textbf{9 / 9} --- QKV proved \emph{exact} and \emph{tolerance}, in \emph{scalar} and
  \emph{torch-native} encodings; each clean target paired with a refuted mutant; \textbf{plus a
  reassociating fusion refuted with a counterexample} (Bitwuzla, fixed dims).
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    \texttt{reassoc\_fusion\_exact} & reassoc \texttt{==} (\textbf{unsound FP}) & \bad\ 1-ULP c.ex. & 15 s \\
    \bottomrule
  \end{tabular}
...
\begin{frame}{Empirical check: we encoded it in Lean too}
  Same QKV equivalence in \textbf{Lean 4} (agent = LLM-authored proof):
  \begin{itemize}
    \item The order-preserving fusion is a \textbf{definitional identity} $\Rightarrow$ proved by
      \texttt{rfl} for \textbf{all dimensions}, over \texttt{Int} \emph{and} \texttt{Float}.
    \item A \textbf{reassociating} fusion is sound over $\mathbb{Z}/\mathbb{R}$ (\texttt{omega}/\texttt{ring})
      but \textbf{false in Float}; core Lean (opaque \texttt{Float}) can't prove it / gives \textbf{no counterexample}.
    \item \textbf{Verified with a real FP library:} \emph{FloatSpec} proves the fusion over genuine
      \textbf{IEEE-754 rounding} ($\forall$ dims, \texttt{rfl}) --- Lean \emph{does} do bit-precise FP ---
      but FloatSpec is a \textbf{noncomputable} proof model, so still \textbf{no concrete eval /
      counterexample}; ESBMC synthesises one.
  \end{itemize}
  \vspace{-1mm}
  ...
  \textbf{\texttt{--floatbv} wins bit-precise FP {+} counterexamples}. Verified in \texttt{lean/floatspec/}, \texttt{lean/Bits.lean}.}
\end{frame}
...
\begin{frame}{Related work: translation validation (Alive2)}
  ...
  the ITP (Lean) end of the spectrum. \textbf{ITP side:} \emph{TorchLean}
  (\texttt{github.com/nktkt/leanx}) formalises NNs in Lean 4 with IEEE-754 binary32 --- Lean
  \emph{can} do bit-precise FP, with a dedicated library.}
\end{frame}

% ---------------------------------------------------------------
\begin{frame}{Which tool for \emph{this} task? (original $\equiv$ optimised PyTorch)}
  \textbf{Recommendation: ESBMC (BMC), default \texttt{--floatbv}.} It matches the task:
  \begin{itemize}
    \item \good\ \textbf{Checks the real code} --- the program \emph{is} the spec; no re-encoding of semantics.
    \item \good\ \textbf{Bit-precise IEEE-754, built in} --- exactly where fusion breaks (reassociation, rounding); plain reals answer the \emph{wrong} question, and a Lean IEEE-754 library needs manual proofs {+} gives no counterexamples.
    \item \good\ \textbf{Counterexamples} --- a concrete failing tensor when the rewrite is wrong (Lean gives none); e.g.\ \texttt{reassoc\_fusion\_exact} yields a 1-ULP witness.
    \item \good\ \textbf{Push-button / CI-able} --- gate optimisations automatically.
  \end{itemize}
  \textbf{Choose Lean (ITP)} only for the \emph{different} goal of a \textbf{size-general,
  all-shapes theorem over reals} --- at higher encoding/proof cost, no FP, no counterexamples.
  \begin{block}{}
    \centering Caveats: ESBMC is \textbf{bounded} (verify the shapes you ship) and \textbf{OM-dependent}.
  \end{block}
\end{frame}
