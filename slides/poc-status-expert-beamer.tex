...
\begin{frame}{Empirical check: we encoded it in Lean too}
  ...
  \begin{itemize}
    ...
    \item A \textbf{reassociating} fusion is sound over $\mathbb{Z}/\mathbb{R}$ (\texttt{omega}/\texttt{ring})
      but \textbf{false in Float}. In \emph{core} Lean (opaque \texttt{Float}) you can't prove it and
      get \textbf{no counterexample}; a Lean IEEE-754 library (FloatSpec / TorchLean) enables FP
      reasoning but still no auto-witness --- ESBMC synthesises one.
  \end{itemize}
  \begin{center}\small
