...
\begin{frame}{Empirical check: we encoded it in Lean too}
  ...
  \begin{tabular}{@{}lll@{}}
    \toprule
     & \textbf{Lean (ITP)} & \textbf{ESBMC (BMC)} \\
    \midrule
    Order-preserving QKV & \texttt{rfl}, $\forall$ dims, $\sim$5 s & per-shape, bit-FP, $\sim$110 s \\
    Reassociating fusion & unsound for FP, \textbf{no c.ex.} & \textbf{finds FP counterexample} \\
    \bottomrule
  \end{tabular}
  \end{center}
  {\footnotesize ITP wins generality; BMC wins bit-precise FP {+} counterexamples --- complementary.
  (ESBMC's \texttt{--ir} reaches $\sim$3 s on the scalar proof, $\approx$ Lean, but abstracts FP and
  diverges on refutation/OM.) Artefact {+} numbers: \texttt{lean/}.}
\end{frame}
